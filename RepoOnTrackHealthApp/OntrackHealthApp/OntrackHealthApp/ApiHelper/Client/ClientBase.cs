...
namespace OntrackHealthApp.ApiHelper.Client
{
    public abstract class ClientBase
    {
        ...
        protected async Task<TResponse> PostJsonEncodedContentAsync<TResponse, TModel>(string url, TModel model)
            where TModel : ApiModel
            where TResponse : ApiResponse<string>, new()
        {
            using (var apiResponse = await _iApiClient.PostJsonEncodedContentAsync(url, model))
            {

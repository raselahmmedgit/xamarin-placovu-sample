...
namespace OntrackHealthApp.ProfessionalProfile
{
	...
	public partial class ProfessionalPatientPage : ContentPage
	{
        ...
        private bool IsProfessionalPatientPage = false;
        ...
        public ProfessionalPatientPage(long practiceDivision, long practiceDivisionUnit)
        {
            InitializeComponent();
            ...
            this.IsProfessionalPatientPage = true;
            this.IsNursePatientInfoPatientViewPageNew = false;

            ReLoadData();
        }

        public ProfessionalPatientPage(long practiceDivisionUnit, DateTime? pastDay)
        {
            InitializeComponent();
            _iTokenContainer = new TokenContainer();
            Title = _iTokenContainer.ApiPracticeName;
        ...
        public ProfessionalPatientPage(long practiceDivisionUnit)
        {
            InitializeComponent();
            _iTokenContainer = new TokenContainer();
            Title = _iTokenContainer.ApiPracticeName;
        ...
        public void ReLoadData()
        {
            try
            {
                ...
                PageViewModel.ReDownloadProfessionalPatientPage();
        ...
        public async void NavigationToDestination(SurgicalConciergePatientViewModel surgicalConciergePatientViewModel)
        {
            ...
            {
                //await Navigation.PushAsync(new ProfessionalPatientEmailPage(selectedPatient, (long)PracticeDivisionUnit.Pathology));
                await Navigation.PushAsync(new ProfessionalPatientEmailPage(selectedPatient, Convert.ToInt16(PracticeDivisionDest), (long)PracticeDivisionUnit.Pathology, this));
            }
            else if (PracticeDivisionUnitId.GetOperatingRoomDivisionUnitId().Contains(PracticeDivisionUnitDest))
            {
                await Navigation.PushAsync(new ProfessionalRecipientPage(selectedPatient, PracticeDivisionUnitDest, this));
            }
            ...
            else if (PracticeDivisionUnitId.GetPreSurgerySummaryDivisionUnitId().Contains(PracticeDivisionUnitDest))
            {
                ...
                if (patientPreSurgerySummaryViewModel != null)
                {
                    if (patientPreSurgerySummaryViewModel.IsAlreadyInsertedSummary)
                    {
                        ...
                        await Navigation.PushAsync(new ProfessionalPreSurgerySummaryPage(selectedPatient, patientPreSurgerySummaryViewModel));
                    }
                    else
                    {
                        await Navigation.PushAsync(new ProfessionalPreSurgerySummaryAddPage(selectedPatient, patientPreSurgerySummaryViewModel));
                    }
                }
                else
                {
        ...
        private async void ShowAddPatientModal(object sender, EventArgs e)
        {
            using (UserDialogs.Instance.Loading(""))
            {
                ...
                await Navigation.PushAsync(new ProfessionalPatientAddNew(this, PracticeDivisionDest, PracticeDivisionUnitDest, false));
                //await Navigation.PushModalAsync(new ProfessionalPatientAddNew(this, PracticeDivisionDest, PracticeDivisionUnitDest, false));
            }
        }
        ...
        public async void OpenSurgicalDetailPage(SurgicalConciergeViewModel surgicalConciergeViewModel, SurgicalConciergePatientViewModel model)
        {
            try
            {
                ProfessionalOperatingPage surgicalConciergeDetail = new ProfessionalOperatingPage(surgicalConciergeViewModel, model);
                surgicalConciergeDetail.LoadData();
                await Navigation.PushAsync(surgicalConciergeDetail);
            }
            catch (Exception)
        ...
        private void ShowToolbar()
        {
            if (Device.RuntimePlatform == Device.iOS)
            {
                ...
                var toolbarItem = new ToolbarItem("AddPatient", null, () =>
                {
                    ...
                    Navigation.PushModalAsync(new ProfessionalPatientAdd(this, false));
        ...
        private async void ButtonContinueExtended_Clicked(object sender, EventArgs e)
        {
            ...
            using (UserDialogs.Instance.Loading(""))
            {
                try
                {
                    ...
                    {
                        //await Navigation.PushAsync(new ProfessionalPatientAddNew(this, PracticeDivisionUnitDest, false));
                    }
                    ...
                    {
                        //await Navigation.PushAsync(new ProfessionalPathologyPage(selectedPatient));
                        //await Navigation.PushAsync(new ProfessionalPatientEmailPage(selectedPatient, (long)PracticeDivisionUnit.Pathology));
                        await Navigation.PushAsync(new ProfessionalPatientEmailPage(selectedPatient, Convert.ToInt16(PracticeDivisionDest), (long)PracticeDivisionUnit.Pathology, this));
                    }
                    else if (PracticeDivisionUnitId.GetOperatingRoomDivisionUnitId().Contains(PracticeDivisionUnitDest))
            ...
            using (UserDialogs.Instance.Loading(""))
            {
                try
                {
                    ...
                    if (surgicalConciergePatientViewModel != null)
                    {
                        ...
                        await Navigation.PushAsync(new ProfessionalPatientAddNew(this, PracticeDivisionDest, PracticeDivisionUnitDest, selectedSurgicalConciergePatientViewModel.PatientProcedureDetailId, surgicalConciergePatientViewModel, false));
                        //await Navigation.PushModalAsync(new ProfessionalPatientAddNew(this, PracticeDivisionDest, PracticeDivisionUnitDest, selectedSurgicalConciergePatientViewModel.PatientProcedureDetailId, surgicalConciergePatientViewModel, false));
                    }
                    else
                    {

...
namespace OntrackHealthApp.ProfessionalProfile
{
    ...
    public partial class ProfessionalPreSurgerySummaryEditPage : ContentPage
    {
        ...
        private Guid _preSurgerySummaryId;
        private int _cancerLocationTemplateType;

        private SurgicalConciergeRestApiService _restApiService;

        public ProfessionalPreSurgerySummaryEditPage(SurgicalConciergePatientViewModel surgicalConciergePatientViewModel, PatientPreSurgerySummaryViewModel patientPreSurgerySummaryViewModel)
        {
            InitializeComponent();
            ...
            _preSurgerySummaryId = patientPreSurgerySummaryViewModel.PreSurgerySummaryId;
            _cancerLocationTemplateType = (int)patientPreSurgerySummaryViewModel.CancerLocationTemplateType;
            _restApiService = new SurgicalConciergeRestApiService();
            puopulateCancerSummaryData(patientPreSurgerySummaryViewModel);

        }

        private void puopulateCancerSummaryData(PatientPreSurgerySummaryViewModel patientPreSurgerySummaryViewModel)
        {
            PreopPsaTextBox.Text = patientPreSurgerySummaryViewModel.PreopPsa != null ? patientPreSurgerySummaryViewModel.DisplayPreopPsa : "";
            VolumeTextBox.Text = patientPreSurgerySummaryViewModel.Volume != null ? patientPreSurgerySummaryViewModel.DisplayVolume : "";
            IPSSTextBox.Text = patientPreSurgerySummaryViewModel.IntProstateSymptomScore != null ? patientPreSurgerySummaryViewModel.DisplayIntProstateSymptomScore : "";
            IIEF5TextBox.Text = patientPreSurgerySummaryViewModel.IntIndexErectileFunction5 != null ? patientPreSurgerySummaryViewModel.DisplayIntIndexErectileFunction5 : "";

            NoteTextBox.Text = patientPreSurgerySummaryViewModel.Notes != null ? patientPreSurgerySummaryViewModel.Notes.ToString() : "";
            ...
            foreach (var item in buttonList)
            {
                ...
                if (btn.ClassId.Equals("Grade_One_" + patientPreSurgerySummaryViewModel.GleasonScoreOne))
                {
                    btn.BackgroundColor = Color.FromHex("#610094");
                    btn.TextColor = Color.FromHex("#FFF");
                }
            }
            buttonList = gradeTwoStackLayout.Children.Where(c => c.GetType() == typeof(Button));
            foreach (var item in buttonList)
            {
                Button btn = item as Button;
                if (btn.ClassId.Equals("Grade_Two_" + patientPreSurgerySummaryViewModel.GleasonScoreTwo))
                {
                    btn.BackgroundColor = Color.FromHex("#610094");
                    btn.TextColor = Color.FromHex("#FFF");
                }
            ...
            buttonList = stageStackLayoutOne.Children.Where(c => c.GetType() == typeof(Button));
            foreach (var item in buttonList)
            {
                Button btn = item as Button;
                if (btn.ClassId.Equals(patientPreSurgerySummaryViewModel.StageScore))
                {
                    btn.BackgroundColor = Color.FromHex("#610094");
                    btn.TextColor = Color.FromHex("#FFF");
                }
            ...
            buttonList = cancerLocationTemplateTypeStackLayout.Children.Where(c => c.GetType() == typeof(Button));
            foreach (var item in buttonList)
            {
                Button btn = item as Button;
                if (patientPreSurgerySummaryViewModel.CancerLocationTemplateType.HasValue &&
                    btn.ClassId.Equals(patientPreSurgerySummaryViewModel.CancerLocationTemplateType.Value + ""))
                {
                    btn.BackgroundColor = Color.FromHex("#610094");
                    btn.TextColor = Color.FromHex("#FFF");
                }
            ...
            if (patientPreSurgerySummaryViewModel.CancerLocationTemplateType.HasValue && patientPreSurgerySummaryViewModel.CancerLocationTemplateType.Value == 2)
            {
                GenerateCancerLocationViewForTwoItems(int.Parse(CancerLocationTemplateTypeId.Two), patientPreSurgerySummaryViewModel.PreSurgerySummaryId);
            }
            else if (patientPreSurgerySummaryViewModel.CancerLocationTemplateType.HasValue && patientPreSurgerySummaryViewModel.CancerLocationTemplateType.Value == 6)
            {
                GenerateCancerLocationViewForSixItems(int.Parse(CancerLocationTemplateTypeId.Six), patientPreSurgerySummaryViewModel.PreSurgerySummaryId);
            }
            else
            {
                GenerateCancerLocationViewForTwelveItems(int.Parse(CancerLocationTemplateTypeId.Twelve), patientPreSurgerySummaryViewModel.PreSurgerySummaryId);
            }

            GeneratePiradListView(patientPreSurgerySummaryViewModel);
        ...
        {
            Navigation.PushModalAsync(new ProfessionalPreSurgerySummaryPiradEditPage(piradViewModel));
        }
        ...
        public async void OnFinishedButton_Clicked(object sender, EventArgs e)
        {
            ...
            if (InternetConnectHelper.CheckConnection())
            {
                ...
                {
                    ApiExecutionResult result = await UpdatePreSurgerySummary(_patientPreSurgerySummaryViewModel);
                    if (result.Success)
                    {
                        UtilHelper.ShowToastMessage("Data updated successfully");
                        MessagingCenter.Send<ProfessionalPreSurgerySummaryEditPage, PatientPreSurgerySummaryViewModel>(this, "UpdatePreSummary", _patientPreSurgerySummaryViewModel);
                    }
                    else
                    {
                        UtilHelper.ShowToastMessage("Data update failed");
                    }
                }
            }
            else
            ...
            if (InternetConnectHelper.CheckConnection())
            {
                using (UserDialogs.Instance.Loading(""))
                {
                    ...
                    if (selectedButton.ClassId.Contains(CancerLocationTemplateTypeId.Twelve))
                    {
                        ...
                        if (_cancerLocationTemplateType == _patientPreSurgerySummaryViewModel.CancerLocationTemplateType)
                            GenerateCancerLocationViewForTwelveItems(int.Parse(CancerLocationTemplateTypeId.Twelve), _preSurgerySummaryId);
                        else
                    ...
                    else if (selectedButton.ClassId.Contains(CancerLocationTemplateTypeId.Six))
                    {
                        ...
                        if (_cancerLocationTemplateType == _patientPreSurgerySummaryViewModel.CancerLocationTemplateType)
                            GenerateCancerLocationViewForSixItems(int.Parse(CancerLocationTemplateTypeId.Six), _preSurgerySummaryId);
                        else
                    ...
                    else if (selectedButton.ClassId.Contains(CancerLocationTemplateTypeId.Two))
                    {
                        ...
                        if (_cancerLocationTemplateType == _patientPreSurgerySummaryViewModel.CancerLocationTemplateType)
                            GenerateCancerLocationViewForTwoItems(int.Parse(CancerLocationTemplateTypeId.Two), _preSurgerySummaryId);
                        else
        ...
        {
            await Navigation.PushModalAsync(new ProfessionalPreSurgerySummaryPiradAddPage(_patientPreSurgerySummaryViewModel));
        }
            ...
            if (_patientPreSurgerySummaryViewModel.CancerLocationViewModels != null && _patientPreSurgerySummaryViewModel.CancerLocationViewModels.Any())
            {
                foreach (var cancerLocation in _patientPreSurgerySummaryViewModel.CancerLocationViewModels)
                {
                    Button cancerLocationButton = new Button
                    {
                        ...
                        TextColor = cancerLocation.IsChecked ? Color.FromHex("#FFF") : Color.FromHex("#000"),
                        FontSize = (IsiOSName5s == true ? 14 : 18),
                        BackgroundColor = cancerLocation.IsChecked ? Color.FromHex("#610094") : Color.FromHex("#FFF"),
        ...
        {
            await Navigation.PushModalAsync(new ProfessionalCancerLocationAddPage(cancerLocation));
        }

        private void SubscribeToEvents()
        {
            MessagingCenter.Unsubscribe<ProfessionalCancerLocationAddPage>(this, "UpdateCancerLocation");
            MessagingCenter.Unsubscribe<ProfessionalPreSurgerySummaryPiradAddPage>(this, "AddPiradLesion");
            MessagingCenter.Unsubscribe<ProfessionalPreSurgerySummaryPiradEditPage>(this, "UpdatePiradLesion");
            MessagingCenter.Unsubscribe<ProfessionalPreSurgerySummaryPiradEditPage, PatientPreSurgerySummaryPiradViewModel>(this, "DeletePiradLesion");

            MessagingCenter.Subscribe<ProfessionalCancerLocationAddPage>(this, "UpdateCancerLocation", (arg) =>
            {
                UpdateCancerLocationMaps();
            });

            MessagingCenter.Subscribe<ProfessionalPreSurgerySummaryPiradAddPage>(this, "AddPiradLesion", (arg) =>
            {
                GeneratePiradListView(_patientPreSurgerySummaryViewModel);
            });

            MessagingCenter.Subscribe<ProfessionalPreSurgerySummaryPiradEditPage>(this, "UpdatePiradLesion", (arg) =>
            {
                GeneratePiradListView(_patientPreSurgerySummaryViewModel);
            });

            MessagingCenter.Subscribe<ProfessionalPreSurgerySummaryPiradEditPage, PatientPreSurgerySummaryPiradViewModel>(this, "DeletePiradLesion", (sender, deletedPirad) =>
            {
        ...
        private void RemovePiradItem(PatientPreSurgerySummaryPiradViewModel deletedPirad)
        {
            ...
            foreach (var item in _patientPreSurgerySummaryViewModel.PatientPreSurgerySummaryPiradViewModels)
            {
                if (item.PiradLesionId.Equals(deletedPirad.PiradLesionId))
                {
                    item.IsDeleted = true;
                }
            }
        ...
        private async Task<ApiExecutionResult> UpdatePreSurgerySummary(PatientPreSurgerySummaryViewModel model)
        {
            ApiExecutionResult apiExecutionResult = new ApiExecutionResult();
            try
            {
                apiExecutionResult = await _restApiService.UpdatePreSurgerySummary(model);
            }
            catch (Exception ex)
            {

...
{
    public partial class SurgicalConciergePatientView : ContentPage
    {
        ...
        private bool IsSurgicalConciergePatientView = false;
        ...
        public SurgicalConciergePatientView(long practiceDivision, long practiceDivisionUnit)
        {
            InitializeComponent();
            ...
            this.IsSurgicalConciergePatientView = true;
            this.IsNursePatientInfoPatientViewPageNew = false;

            ReLoadData();
        }

        public SurgicalConciergePatientView(long practiceDivisionUnit, DateTime? pastDay)
        {
            InitializeComponent();
            _iTokenContainer = new TokenContainer();
            //Title = _iTokenContainer.ApiPracticeName;
        ...
        private async void PatientViewSelected(object sender, SelectedItemChangedEventArgs e)
        {
            ...
            using (UserDialogs.Instance.Loading(""))
            {
                try
                {
                    ...
                    if (PracticeDivisionDest == (int)PracticeDivisionId.PatientRegistration) //PatientRegistration
                    {
                        SurgicalConciergePatientViewModel surgicalConciergePatientViewModel = await restApiService.EditSurgicalConciergePatientProfile(selectedPatient.PatientProfileId, selectedPatient.PatientProcedureDetailId, Convert.ToInt16(PracticeDivisionDest), Convert.ToInt16(PracticeDivisionUnitDest));
                        if (surgicalConciergePatientViewModel != null)
                        {
                            await Navigation.PushAsync(new SurgicalConceirgePatientAddNew(this, PracticeDivisionDest, PracticeDivisionUnitDest, selectedPatient.PatientProcedureDetailId, surgicalConciergePatientViewModel, false));
                        }
                        else
                        {
                            await DisplayAlert(AppConstant.NotFound, AppConstant.DisplayAlertErrorMessage, AppConstant.DisplayAlertErrorButtonText);
                        }
                        //await Navigation.PushAsync(new SurgicalConceirgePatientAddNew(this, PracticeDivisionUnitDest));
                    }
                    else if (PracticeDivisionUnitId.GetScgNursingRoundDivisionUnitId().Contains(PracticeDivisionUnitDest))  //NursingRound
                    {
                        await Navigation.PushAsync(new SurgicalConciergeRecipientPage(selectedPatient, PracticeDivisionUnitDest, this));
